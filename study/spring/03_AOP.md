...
- 횡단 관심사 : 로그 남기기, 실행 시간 측적, 트랜잭션 관리, 보안 등 비즈니스 로직에 공통으로 들어가는 기능
...
AOP는 이러한 부가적인 코드들을 핵심 비즈니스 로직과 분리하여 한 곳에서 관리할 수 있고, 필요한 곳에 공통 적용할 수 있다.
...
즉, 로그 start, end라고 주석이 있지만 그 부분은 사라지고 핵심 비즈니스 로직 코드만 남게 된다.
...
프록시 객체를 만드는 기능은 Spring의 BeanPostProcessor에 의해서 프록시 객체인지 확인하고 생성하게 된다.
...
스프링에는 빈 생성 전후에 후처리를 할 수 있다. 공식 문서에도 Auto-proxy 기능이 이 지점에서 동작한다고 설명한다. (bean post processor infrastructure)

그래서 우리가 @Autowired로 받는 객체는 걷보기엔 OrderService 같아도, 실제로는 프록시인 경우가 많다.
...
Spring AOP의 auto-proxy는 "selected bean definitions"를 자동 프록시로 감싼다고 설명한다.
...
#### 프록시 생성 방식
그렇다면 프록시는 어떻게 만들어질까. Spring AOP는 두가지 방식으로 프록시를 만든다.
...
스프링은 기본으로 CGLIB 프록시를 사용한다. JDK 프록시를 사용하려면 설정이 필요하다.
...
**포인트 컷에 해당하지 않는 메소드는?**
포인트 컷에 해당하지 않는 메소드는 프록시 객체를 거치지만 interceptor/advice 체인데 들어가지 않는다. 매칭되지 않은 메서드는 별도 어드바이스 없이 바로 타킷 메소드로 위임된다. Spring AOP는 Advisor를 기반으로 프록시를 만들고, 메소드 매칭은 poincut의 maches(Method, Class)같은 방식으로 판단한다.

```
빈이 AOP 후보인가?  → yes → 프록시 생성
현재 호출한 메서드가 포인트컷에 맞는가? → yes면 advice 실행, no면 그냥 통과
```
프록시 생성 시점에는 Spring이 해당 빈에 사용할 Advisor 목록을 구성하는데, AbstractAutoProxyCreator는 bean에 대한 advisors를 결정하고 프록시를 만든다. 다만 실제 호출 시에는 그 advisor 안의 Method/matcher / poincut 으로 현재 호출 메소드가 적용 대상인지 다시 판단한다.


### Spring AOP 문제점
Spring AOP는 프록시 기반이기 때문에 다음과 같은 문제점이 발생한다.

1. self-Invocation 문제 : 같은 클래스 내에서 ```@Trasactional```이 없는 메소드가 있는 메소드를 호출하면 트랜잭션이 적용되지 않는다. 프록시를 거치지 않고 실행된 메소드가 this를 통해 직접 호출하기 때문이다.
...
2. @PointCut : 어디에 이 기능을 적용할지 필터링하는 규칙을 정하는 곳. 위에 예시처럼 한 곳에 규칙을 정의 해두면 아래 ```Before```, ```Around``` 에서 해당 규칙을 재사용할 수 있다.
